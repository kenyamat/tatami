RSpec.describe Tatami::Models::Csv::Header do
  ...
  describe '.get_parent' do
    context 'when limit values are passed' do
      ...
      context 'index is 31' do
        let(:depth) { 1 }
        let(:index) { 31 }
        it { expect { subject }.to raise_error(Tatami::Parsers::WrongFileFormatError, /Invalid header structure/) }
      end
    end
    ...
    context 'when structure is invalid' do
      ...
      it { expect { subject }.to raise_error(Tatami::Parsers::WrongFileFormatError, /depth <= header.depth/) }
    end
  end
  ...
  describe '.get_bool' do
    ...
    context 'xxx' do
      let(:value) { 'xxx' }
      it { expect { subject }.to raise_error(Tatami::Parsers::WrongFileFormatError, /Invalid Data Format/) }
    end
  end
